...
// ====== Drink timing (DRIVES CHALICE ANIMATION) ======
var drink_t = (variable_global_exists("_drinking_timer") ? global._drinking_timer : 0);

// Rising edge = new sip → record fresh peak for normalization
if (drink_t > prev_drink_timer) {
    drink_timer_max = drink_t; // peak frames for this sip
}
...
if (active_anim_index != -1) {
    ...
    if (moon_dir[i] > 0 && moon_frame[i] >= last) {
        moon_state[i] = 1; moon_frame[i] = 0; moon_dir[i] = 0;
        display_hp = min(display_hp + 1, max_hp_cache);
        active_anim_index = -1;
    } else if (moon_dir[i] < 0 && moon_frame[i] <= 0) {
        moon_state[i] = 0; moon_frame[i] = last; moon_dir[i] = 0;
